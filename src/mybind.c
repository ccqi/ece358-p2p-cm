 ...
 *
 * addr -- a pointer to struct sockaddr_in. mybind() works for AF_INET sockets
 *only.
 * Note that addr is and in-out parameter. That is, addr->sin_family and
 * addr->sin_addr are assumed to have been initialized correctly before the
 *call.
 * Also, addr->sin_port must be 0, or the call returns with an error. Up on
 *return,
 * addr->sin_port contains, in network byte order, the port to which the call
 *bound
 * sockfd.
 *
 * returns int -- negative return means an error occurred, else the call
 *succeeded.
 */
int mybind(int sockfd, struct sockaddr_in *addr) {
    ...
    if (addr->sin_port != 0) {
        fprintf(stderr, "mybind(): addr->sin_port is non-zero. Perhaps you "
                        "want bind() instead?\n");
        return -1;
    }
    ...
    for (p = PORT_RANGE_LO; p <= PORT_RANGE_HI; p++) {
        ...
        int b = bind(sockfd, (const struct sockaddr *)addr,
    ...
    /* Note: upon successful return, addr->sin_port contains, in network byte
     * order, the
